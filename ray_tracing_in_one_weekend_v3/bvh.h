...
bvh_node::bvh_node(const std::vector<shared_ptr<hittable>>& src_objects, size_t start, size_t end, double time0, double time1) {
    ...
    auto comparator = (axis == 0) ? box_x_compare : (axis == 1) ? box_y_compare : box_z_compare;  // 随机选取一个维度

    size_t object_span = end - start;  // 编号

    if (object_span == 1) {  // 只剩下一个
        left = right = objects[start];
    }
    else if (object_span == 2) {
        if (comparator(objects[start], objects[start + 1])) { // 只剩下两个就不用排序
            left = objects[start];
        ...
        std::sort(objects.begin() + start, objects.begin() + end, comparator);  // 排序
